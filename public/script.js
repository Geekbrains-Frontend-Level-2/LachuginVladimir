class Renderer {
  ...
  class Cart extends Renderer {
    ...
    add (data) {
      const ExistedItem = this._items.filter(item.id === data.id)[0]
      if (ExistedItem) {
    ...
    initTemplate () {
      ...
      if (!this._template.className) {
        ...
        this.root.addEventListener('click', this.toggle.bild(this))
      }
      ...
      if (this._items.length) {
        ...
        this._template.innerHTML = `<div class="cart__empty">Вы пока не добавили товары в корзину</div>`
      }
    }
    ...
    totalCostTemplate () {
      ...
      summaryContainer.innerHTML = this.totalPrice
  ...
  class Item extends Renderer {
    ...
    initTemplate () {
      ...
      ` 
      const btn = this.template.querySelector('cart-button')
  ...
  class ItemList extends Renderer {
    ...
    initTemplate () {
      ...
      this._template.className = 'goods-list'
